...
def _handle_connection(manager: LogitechDevice, parser: ProtocolParser, sock: socket.socket, ver_string: str, event: Event) -> None:
    """
...
def _load_new_plane_if_detected(manager: LogitechDevice) -> None:
    """
...
def _sock_err_handler(manager: LogitechDevice, start_time: float, ver_string: str, support_iter: Iterator[str], exp: Exception) -> None:
    """
...
def dcspy_run(lcd_type: str, event: Event, fonts_cfg: FontsConfig, skip_lcd=False) -> None:
    ...
    with _prepare_socket() as dcs_sock:
        ...
        manager: LogitechDevice = getattr(import_module('dcspy.logitech'), lcd_type)(parser=parser, sock=dcs_sock, fonts=fonts_cfg, skip_lcd=skip_lcd)
