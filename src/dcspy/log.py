...
def config_logger(logger: Logger, verbose: bool = False) -> None:
    ...
    file_hand.setFormatter(Formatter(fmt='%(asctime)s | %(name)-17s | %(levelname)-8s | %(threadName)-10s | %(message)s / %(funcName)s:%(lineno)d'))
    ...
    stream_hand.setFormatter(Formatter(fmt='%(levelname)-8s | %(message)s'))
