...
CLEAN_BEFORE_LOAD_PLANE = False
CLEAN_WHILE_WAIT_FOR_DATA = False
...
def _handle_connection(logi_device: LogitechDevice, parser: ProtocolParser, sock: socket.socket, ver_string: str, event: Event) -> None:
    ...
    """
    start_time = time()
    global CLEAN_BEFORE_LOAD_PLANE
    global CLEAN_WHILE_WAIT_FOR_DATA
    ...
    while not event.is_set():
        try:
            ...
            if CLEAN_BEFORE_LOAD_PLANE:
                LOG.debug(f'Before: {CLEAN_WHILE_WAIT_FOR_DATA=} {CLEAN_BEFORE_LOAD_PLANE=}')
                logi_device.clear(true_clear=True)
                CLEAN_BEFORE_LOAD_PLANE = False
                CLEAN_WHILE_WAIT_FOR_DATA = True
...
def _sock_err_handler(logi_device: LogitechDevice, start_time: float, ver_string: str, support_iter: Iterator[str], exp: Exception) -> None:
    ...
    global CLEAN_WHILE_WAIT_FOR_DATA
    global CLEAN_BEFORE_LOAD_PLANE
    if CLEAN_WHILE_WAIT_FOR_DATA:
        LOG.debug(f'Main loop socket error: {exp}')
        LOG.debug(f'After clean: {CLEAN_WHILE_WAIT_FOR_DATA=} {CLEAN_BEFORE_LOAD_PLANE=}')
        logi_device.clear(true_clear=True)
        CLEAN_BEFORE_LOAD_PLANE = True
        CLEAN_WHILE_WAIT_FOR_DATA = False
